...
>
				...
				{ 
					...
					$query = "
								SELECT 
									...
									, ( 
										SELECT box  FROM ( 
										SELECT nombre BOX , row_number() over(ORDER BY orden ) fila FROM boxes WHERE ESTADO = 1 ORDER BY orden 
				...
				,LEFT( CAST( hora AS VARCHAR(100)), 5 )  || 'hs | ' || left(cast(hora + ( coalesce( tiempo , 0 ) / 0.5 )	* INTERVAL '30 minute' as varchar(100)),5) || 'hs' rango_horario 
								...
								(SELECT CASE WHEN count(DISTINCT nombre ) > 0 then 'SI' ELSE 'NO' END  
								FROM fichas t2 
								WHERE t2.nombre = t1.nombre 
								AND t2.vehiculo = t1.vehiculo 
								and t2.vin = t1.vin 
								AND estado = '0' 
								AND t2.fecha BETWEEN t1.fecha - INTERVAL '10 day' AND t1.fecha - INTERVAL '1 day' ) reAgendado
								
								FROM public.fichas t1 
								WHERE fecha BETWEEN '" . $fecha1 . "' AND '" . $fecha2 . "' 
								AND lower(t1.nombre) NOT LIKE '%ocupado%'
								AND lower(t1.nombre) NOT LIKE '%vacacion%'
