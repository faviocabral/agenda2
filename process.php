...
/*//$_SESSION['pagina']='form-empleado.php';
...
if($mydb->dbConnect())
{
 				...
 				$registros=$mydb->dbSelect("select * from fichas order by id_ficha"); //  
				echo $registros[0]->nombre;
/*			 	if($_POST['codigo'])
				{
					$_SESSION['codigo_empleado']=trim(strip_tags($_POST['codigo']));
				}
				  
				if($_SESSION['codigo_empleado']<>"")
				{
					$rs03=$mydb->dbSelect("select * from empleados where emp_codigo='$_SESSION[codigo_empleado]'"); //  	
				} 
}
 */
?>
...
switch ($accion)
{
	...
	case 'eliminarFicha':
			...
			if ($res= pg_fetch_array($exq))
			{
				...
				$retorna = "Usuario no puede eliminar o modificar registro".$qupdate.$tab;	
			}
	break;
	case 'nuevaFicha':
			$xcolumna   = 		$_POST['xcolumna'];
		...
		if($base == 'mpy'){ 
			...
			"
				...
				from ( 
					...
					select top 15
					oinv.cardcode customer  
					,  replace( replace( replace( oinv.CardName, char(34) , '' ) , char(39) , '' ), ';' , '') as custmrName 
					,isnull( t1.itemcode, '') as itemCode
					,isnull( vehiculo, '') as itemName
					, replace(Phone1, '''', '') +''+ replace(Phone2, '''', '') telefono
					,replace(e_mail , '@' , '&') mail 
					,isnull( t1.marca, '') marca  
					,isnull(t1.modelo, '') modelo 
					,isnull(t1.color , '') color  
					,ROW_NUMBER() over(partition by oinv.cardcode order by oinv.cardcode , oinv.createDate desc ) hijo 
					,isnull( t1.itemcode , '') chassis  
					from  garden_mpy.dbo.OCRD WITH(NOLOCK)  
					join garden_mpy.dbo.oinv with(nolock) on ocrd.cardcode = oinv.cardcode 
					join garden_mpy.dbo.inv1 with(nolock) on oinv.docentry = inv1.docentry 
					join (select t2.name color, t3.Name marca, t4.Name modelo , t1.itemcode , U_Chassis chassis , t1.ItemName vehiculo 
										from garden_mpy.dbo.oitm t1 with(nolock) 
										join garden_mpy.dbo.[@COLOR] t2 on t1.U_Color = t2.Code 
										join garden_mpy.dbo.[@MARCAS] t3 on t1.U_Marca = t3.Code 
										join garden_mpy.dbo.[@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on t1.itemcode = inv1.itemcode 
					where ( oinv.cardcode  like '%$CodigoCliente%' or oinv.CardName like '%$CodigoCliente%' or t1.itemCode like '%$CodigoCliente%' ) 
					and oinv.u_conceptoFactura = 'vhe'
		
					union all 
		
					select top 15
						customer  
						,  replace( replace( replace( custmrName, char(34) , '' ) , char(39) , '' ), ';' , '') as custmrName 
						,isnull( internalSN, '') as itemCode
						...
						,isnull(t1.modelo, '') modelo
						,isnull(t1.color , '') color
						,ROW_NUMBER() over(partition by customer order by customer , oins.createDate desc ) hijo
						,isnull( manufSN, '') chassis 
					from  garden_mpy.dbo.OCRD WITH(NOLOCK) left outer join oins with(nolock) on ocrd.cardcode = oins.customer
					left outer join (select t2.name color, t3.Name marca, t4.Name modelo , right(ItemCode, 12) itemcode 
										from garden_mpy.dbo.oitm t1 with(nolock)
										join garden_mpy.dbo.[@COLOR] t2 on t1.U_Color = t2.Code 
										join garden_mpy.dbo.[@MARCAS] t3 on t1.U_Marca = t3.Code 
										join garden_mpy.dbo.[@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on right(oins.itemCode , 12) = t1.itemcode 
		
					where ( customer like '%$CodigoCliente%' or custmrName like '%$CodigoCliente%' or oins.itemCode like '%$CodigoCliente%' ) 
					order by customer , ROW_NUMBER() over(partition by customer order by customer , oins.createDate desc )
			...
			"
				...
				from ( 
					...
					,isnull(t1.color , '') color  
					,ROW_NUMBER() over(partition by oinv.cardcode order by oinv.cardcode , oinv.createDate desc ) hijo 
					,isnull( t1.chassis , '') chassis  
					from  gardensa.dbo.OCRD WITH(NOLOCK)  
					join gardensa.dbo.oinv with(nolock) on ocrd.cardcode = oinv.cardcode 
					join gardensa.dbo.inv1 with(nolock) on oinv.docentry = inv1.docentry 
					join (select t2.name color, t3.Name marca, t4.Name modelo , itemcode , U_Chassis chassis , t1.ItemName vehiculo 
										from gardensa.dbo.oitm t1 with(nolock) 
										join gardensa.dbo.[@COLOR] t2 on t1.U_Color = t2.Code 
										join gardensa.dbo.[@MARCAS] t3 on t1.U_Marca = t3.Code 
										join gardensa.dbo.[@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on t1.itemcode = inv1.itemcode 
					where ( oinv.cardcode  like '%$CodigoCliente%' or oinv.CardName like '%$CodigoCliente%' or t1.itemCode like '%$CodigoCliente%' ) 
					and oinv.u_conceptoFactura = 'vhe'
					...
					select top 15
						...
						,ROW_NUMBER() over(partition by customer order by customer , oins.createDate desc ) hijo
						,isnull( manufSN, '') chassis 
					from  gardensa.dbo.OCRD WITH(NOLOCK) left outer join oins with(nolock) on ocrd.cardcode = oins.customer
					left outer join (select t2.name color, t3.Name marca, t4.Name modelo , right(ItemCode, 12) itemcode 
										from gardensa.dbo.oitm t1 with(nolock)
										join gardensa.dbo.[@COLOR] t2 on t1.U_Color = t2.Code 
										join gardensa.dbo.[@MARCAS] t3 on t1.U_Marca = t3.Code 
										join gardensa.dbo.[@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on right(oins.itemCode , 12) = t1.itemcode 
			...
			"
				...
				from ( 
					...
					join oinv with(nolock) on ocrd.cardcode = oinv.cardcode 
					join inv1 with(nolock) on oinv.docentry = inv1.docentry 
					join (select t2.name color, t3.Name marca, t4.Name modelo , itemcode , U_Chassis chassis , t1.ItemName vehiculo 
										from oitm t1 with(nolock) 
										join [@COLOR] t2 on t1.U_Color = t2.Code 
										join [@MARCAS] t3 on t1.U_Marca = t3.Code 
										join [@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on t1.itemcode = inv1.itemcode 
					where ( oinv.cardcode  like '%$CodigoCliente%' or oinv.CardName like '%$CodigoCliente%' or t1.itemCode like '%$CodigoCliente%' ) 
					and oinv.u_conceptoFactura = 'vhe'
					...
					select top 15
						...
						,ROW_NUMBER() over(partition by customer order by customer , oins.createDate desc ) hijo
						,isnull( manufSN, '') chassis 
					from  OCRD WITH(NOLOCK) left outer join oins with(nolock) on ocrd.cardcode = oins.customer
					left outer join (select t2.name color, t3.Name marca, t4.Name modelo , right(ItemCode, 17) itemcode 
										from oitm t1 with(nolock)
										join [@COLOR] t2 on t1.U_Color = t2.Code 
										join [@MARCAS] t3 on t1.U_Marca = t3.Code 
										join [@MODELOS] t4 on t1.U_Modelo = t4.Code ) t1 on right(oins.itemCode , 17) = t1.itemcode 
