		...
		if($base == 'mpy'){ 
			...
			"
				...
				from ( 
					...
					from (
						select top 15
						replace(oinv.cardcode, ' ', '') customer  
						,replace( replace( replace( oinv.CardName, char(34) , '' ) , char(39) , '' ), ';' , '') as custmrName 
						,isnull( t1.U_Chassis, right(t1.ItemCode, 17)) as itemCode
						,isnull( t1.ItemName , '') as itemName
						,replace(Phone1, '''', '') +''+ replace(Phone2, '''', '') telefono
						,replace(e_mail , '@' , '&') mail 
						,isnull( t3.name, '') marca  
						,isnull(t4.name, '') modelo 
						,isnull(t2.name , '') color  
						,isnull( t1.U_Chassis , '') chassis  
					from  OCRD WITH(NOLOCK) 
						...
						join oitm t1 with(nolock) on t1.ItemCode = inv1.ItemCode 
						join [@COLOR] t2 on t1.U_Color = t2.Code 
						join [@MARCAS] t3 on t1.U_Marca = t3.Code 
						join [@MODELOS] t4 on t1.U_Modelo = t4.Code 
					where OCRD.cardcode  like '%$CodigoCliente%'
					and oinv.u_conceptoFactura = 'vhe'
					and oinv.U_LIIV = 1 
					and isnull(oinv.U_NumeroContrato, 0) <> 0 
					...
					from OCRD WITH(NOLOCK) 
						...
						join [@MARCAS] t3 on t1.U_Marca = t3.Code 
						join [@MODELOS] t4 on t1.U_Modelo = t4.Code 
					where OCRD.CardName like '%$CodigoCliente%'
					and oinv.u_conceptoFactura = 'vhe'
					and oinv.U_LIIV = 1 
					and isnull(oinv.U_NumeroContrato, 0) <> 0 
					...
					from OCRD WITH(NOLOCK)  
						...
						join [@MARCAS] t3 on t1.U_Marca = t3.Code 
						join [@MODELOS] t4 on t1.U_Modelo = t4.Code 
					where t1.ItemCode like '%$CodigoCliente%'
					and oinv.u_conceptoFactura = 'vhe'
					and oinv.U_LIIV = 1 
					and isnull(oinv.U_NumeroContrato, 0) <> 0 
					...
					from OCRD WITH(NOLOCK) 
					join oins with(nolock) on ocrd.cardcode = oins.customer
					join (select t2.name color, t3.Name marca, t4.Name modelo , right(ItemCode, 17) itemcode 
